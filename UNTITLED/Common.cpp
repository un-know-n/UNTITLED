...
void Common::Setup_Colors(){
    //Creation of the main pens and brushes

    Common::Create_DrawSet(Common::Green_Block, Common::Green_Pen, Common::Green_Brush);
    Common::Create_DrawSet(Common::Blue_Block, Common::Blue_Pen, Common::Blue_Brush);
    Common::Create_DrawSet(Common::Red_Block, Common::Red_Pen, Common::Red_Brush);
    Common::Create_DrawSet(Common::Yellow_Block, Common::Yellow_Pen, Common::Yellow_Brush);

    Common::Create_DrawSet(Common::BG_Color, Common::BG_Pen, Common::BG_Brush);
}

//Using 3-rd principle - Polymorphism

void Common::Create_DrawSet(const Color &color, HPEN &pen, HBRUSH &brush) {
    pen = CreatePen(PS_SOLID, 0, RGB(color.R, color.G, color.B));
    brush = CreateSolidBrush(RGB(color.R, color.G, color.B));
}

void Common::Create_DrawSet(unsigned char r, unsigned char g, unsigned char b, HPEN &pen, HBRUSH &brush) {
